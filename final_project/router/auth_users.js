...
const isValid = (username)=>{ 
  ...
  if (userswithsamename.length > 0) {
    return false; // Username is taken (not valid to register)
  } else {
    return true; // Username is available (valid to register)
  }
}
...
regd_users.post("/login", (req, res) => {
  ...
  // Check if the credentials match an existing registered user
  if (authenticatedUser(username, password)) {
    // Generate JWT access token signed with the secret key "access"
    let accessToken = jwt.sign({ data: password }, 'access', { expiresIn: 60 * 60 });
    
    // Save the token to the session object
...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  // Add or overwrite the review for this specific user under the book's ISBN
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (books[isbn]) {
    ...
    if (book.reviews && book.reviews[username]) {
      ...
      return res.status(200).json({ 
        message: `Reviews for ISBN ${isbn} posted by the user ${username} deleted successfully.` 
      });
    } else {
